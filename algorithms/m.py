...
class CostAlgorithm:
    ...
    def start(self):
        ...
        while not (currentNode.isGoal()):
            ...
            if (not (tomPos[1] + 1 >= self.length_world) and currentNode.getState()[tomPos[0], tomPos[1] + 1] != 1 and 
                ...
                son.setNewCost(right)  # به‌روزرسانی هزینه
                son.setTomPos(right)
                son.moveRight(tomPos)
                if (son.avoidGoBack2(right)):
                    stack.append(son)  # اضافه کردن فرزند به استک
                    if (son.getDepth() > depth):
                        depth = son.getDepth()
                vis[tomPos[0]][tomPos[1] + 1] = True
            # بررسی حرکت به چپ
            if (not (tomPos[1] - 1 < 0) and currentNode.getState()[tomPos[0], tomPos[1] - 1] != 1 and 
                    not vis[tomPos[0]][tomPos[1] + 1]):
                ...
                vis[tomPos[0]][tomPos[1] + 1] = True
            # بررسی حرکت به پایین
            if (not (tomPos[0] + 1 >= self.height_world) and currentNode.getState()[tomPos[0] + 1, tomPos[1]] != 1 and 
                    not vis[tomPos[0]][tomPos[1] + 1]):
                ...
                vis[tomPos[0]][tomPos[1] + 1] = True
            # بررسی حرکت به بالا
            if (not (tomPos[0] - 1 < 0) and currentNode.getState()[tomPos[0] - 1, tomPos[1]] != 1 and 
                    not vis[tomPos[0]][tomPos[1] + 1]):
                ...
                if (son.avoidGoBack2(up)):
                    ...
                    if (son.getDepth() > depth):
                        depth = son.getDepth()
                vis[tomPos[0]][tomPos[1] + 1] = True
            # به‌روزرسانی استک: انتخاب گره با کمترین هزینه
            currentNode = self.getNodeMinCost(stack)
            stack.remove(currentNode)  # حذف گره جاری از استک
        ...
        solution = currentNode.recreateSolutionWorld()  # بازسازی مسیر نهایی
